...
DDA <- read_csv(here("Data_In", "DDA", "YPDevolved", "YPD_24hT_72hT-DDA_POS.csv"))
names(DDA)[2] <- "strain"
DDA$line <- unlist(lapply(DDA$name, function(s) strsplit(s,"_")[[1]][3])) 
DDA$line[DDA$line=="5a"] <- 5
...
DDAm$ST <- paste(DDAm$strain, DDAm$transfer, sep="_")

DDA_anc <- subset(DDA, transfer =="0")
DDA_t24 <- subset(DDA, transfer =="24")
DDA_t72 <- subset(DDA, transfer =="72")

#POS Plot with all ancestral and evolved replicates side by side
RAD20 <- ggplot(data=DDAm, mapping=aes(x = strain, y=avgRAD20, col=as.factor(transfer)))+
...
FoG20 <- ggplot(data=DDAm, mapping=aes(x = strain, y=avgFoG20, col=as.factor(transfer)))+
  geom_sina(alpha=0.6,size=2.5, position = position_dodge(width=0.4))+
  #  stat_summary(fun = median, fun.min = median, fun.max = median, geom = "crossbar", width = 0.5) +
  xlab("Strain")+
...
DDA20
ggsave(here("Figures", "PDF", "YPD_24_72_POS-DDA_20.pdf"), DDA20, width = 5, height=5)
...
FoGDiff24 <- c()
FoGDiff72 <- c()
for (i in unique(DDAm$strain)){
  sub <- subset(DDAm, strain==i)
...
diffs24 <- data.frame(strain24, radDiff24, FoGDiff24)
diffs72 <- data.frame(strain72, radDiff72, FoGDiff72)
...
A02 <- subset(DDAm, strain=="A02") #rad
A03 <- subset(DDAm, strain=="A03") #FoG
A10 <- subset(DDAm, strain=="A10") #rad
A12 <- subset(DDAm, strain=="A12") #rad, Fog
A18 <- subset(DDAm, strain=="A18") #Fog
...
diffs24$strain <- factor(diffs24$strain, levels=order)
diffs72$strain <- factor(diffs72$strain, levels=order)

pdf("Figures/PDF/FigureSx-DDA-YPD-24h-72h.pdf", width=5.5, height=6)
par(mfrow=c(2, 1),mar=c(1,1,1,1), oma=c(3, 3.5, 1, 1))
plot(jitter(as.numeric(as.factor(diffs24$strain)), factor=0.2), diffs24$radDiff24, ylim=c(10, -10), xlim=c(0.8,8.2), col="darkmagenta", ylab = "", xaxt="n", yaxt="n")
points(jitter(as.numeric(as.factor(diffs72$strain))+0.2, factor=0.2), diffs72$radDiff72, col="forestgreen")
abline(h=0, lty=2)
...
plot(jitter(as.numeric(as.factor(diffs24$strain)), 0.2), diffs24$FoGDiff24, ylim=c(-1, 1), col="darkmagenta", xlim=c(0.8,8.2),ylab = "Change in Tolerance (FoG20)", xaxt="n", yaxt="n")
points(jitter(as.numeric(as.factor(diffs72$strain))+0.2, factor= 0.2), diffs72$FoGDiff72, col="forestgreen")
abline(h=0, lty=2)
...
DDA_gm_anc <- subset(DDA_gm, transfer == "0")
DDA_gm_evol24 <- subset(DDA_gm, transfer == "24")
DDA_gm_evol72 <- subset(DDA_gm, transfer == "72")
...
pdf("Figures/PDF/Figure3-DDA-POS-FLC.pdf", width=7.5, height=6)
par(mfrow=c(2, 1),mar=c(1,1,1,1), oma=c(3, 3.5, 1, 1))
