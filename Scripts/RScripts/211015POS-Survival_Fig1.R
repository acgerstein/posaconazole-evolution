...
survival_ancFit_72 <- ex_rates %>%
  ...
  geom_errorbar(aes(ymin=avgOD_72h*90-seOD_72h*90, ymax=avgOD_72h*90+seOD_72h*90), width=.2,
                position=position_dodge(.9)) +
  scale_y_continuous("Survival (% replicates)",
                     sec.axis = sec_axis(~./90, name = "Ancestral growth ability (72 h OD)")) +
  xlab("Strain") +
  ylab("Percent survived") +
  theme_bw()
#theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())

ggsave(here("Figures", "Figure1-Survival_AncestralGrowth_72.pdf"), survival_ancFit_72, width = 6, height=3.5)
